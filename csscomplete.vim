...
elseif borders[max(keys(borders))] == 'colon'
  ...
  elseif prop == 'animation'
    let values = ["animation-name", "animation-duration", "animation-timing-function", "animation-delay","animation-iteration-count", "animation-direction"]
  elseif prop == 'animation-name'
    let values = ["none"]
  elseif prop == 'animation-duration'
    let values = ["0s"]
  elseif prop == 'animation-timing-function'
    let values = ["cubic-bezier()", "ease", "ease-in", "ease-out", "linear"]
  elseif prop == 'animation-delay'
    let values = ["0s"]
  elseif prop == 'animation-iteration-count'
    let values = ["1", "infinite"]
  ...
  elseif prop == 'backface-visibility'
	...
	elseif prop =~ 'border-\%(top\|right\|bottom\|left\)-color'
		let values = ["rgb(", "#", "transparent"]
	elseif prop =~ 'border-\%(top\|right\|bottom\|left\)-radius'
		let values = ["0"]
...
ruby rt rp ruby section summary
		if stridx(',a,abbr,acronym,address,area,article,aside,audio,b,base,bdo,bdi,big,blockquote,body,br,button,caption,canvas,cite,code,col,colgroup,command,datalist,dd,del,details,dfn,div,dl,dt,em,embed,fieldset,figure,figcaption,footer,form,head,header,hgroup,h1,h2,h3,h4,h5,h6,hr,html,i,img,input,ins,kbd,keygen,label,legend,li,link,map,mark,meta,meter,nav,noscript,object,ol,optgroup,option,output,p,param,pre,progress,q,rt,rp,ruby,samp,script,select,small,span,strong,style,section,summarysub,sup,table,tbody,td,textarea,tfoot,th,thead,time,title,tr,track,tt,ul,var,video,wbr,', ','.element.',') > -1
